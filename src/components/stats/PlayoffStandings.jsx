...
import { getRoundWins, getStandings } from "../../utils/standingsCalculations";
...
const PlayoffStandings = ({ seasonYear, currentRace, themeMode, onDriverClick }) => {
  ...
  const regularSeasonStandingsData = standingsData.filter(
    (r) => r.race_number === Math.min(currentRace, 26)
  );
  const playoffSeasonStandingsData = standingsData.filter(
    (r) => r.race_number > 26 && r.race_number === currentRace
  );
  const round16StandingsData = standingsData.filter(
    (r) => r.race_number > 26 && r.race_number <= 29
  );
  const round12StandingsData = standingsData.filter(
    (r) => r.race_number > 29 && r.race_number <= 32
  );
  const round8StandingsData = standingsData.filter(
    (r) => r.race_number > 32 && r.race_number <= 35
  );
  ...
  const regularSeasonLeader = (() => {
    ...
    regularSeasonData.forEach((r) => {
      if (
        !lastRaceByDriver[r.driver_name] ||
        r.race_number > lastRaceByDriver[r.driver_name].race_number
      ) {
  ...
  const getHighlight = (driver, index) => {
    ...
    // Champion
    if (row?.champion === 1) {
      return applyZebraTint(isDark ? "#294842" : "#b2dfdb", isDark ? "#243b36" : "#a5d6d5", index);
    }
    ...
    // Locked-in
    if (isLockedIn) {
      return applyZebraTint(isDark ? "#4e342e" : "#ffe0b2", isDark ? "#3e2723" : "#ffd180", index);
    }

    // Promoted
    if (pos <= bubbleCutoff) {
      return applyZebraTint(isDark ? "#355635" : "#dcedc8", isDark ? "#2a4430" : "#c8e6c9", index);
    }

    // Just out
    if (pos <= bubbleCutoff + 4) {
      return applyZebraTint(isDark ? "#5d2e35" : "#f8bbd0", isDark ? "#4a2027" : "#f48fb1", index);
    }
        ...
        <Table stickyHeader>
          <TableHead>
            <TableRow sx={{ bgcolor: "primary.main" }}>
              <TableCell sx={{ fontWeight: "bold", textAlign: "center", cursor: "pointer" }}>
                <TableSortLabel>Playoff Pos</TableSortLabel>
              </TableCell>
              ...
              return (
                <TableRow
                  ...
                  sx={{
                    ...
                    borderBottom:
                      playoffPosMap[driver] === bubbleCutoff ? "3px solid #00000033" : undefined,
                ...
                >
                  ...
                  <TableCell sx={{ textAlign: "center" }}>
                    {getStandings(regularSeasonStandingsData, driver, "wins")}
                  </TableCell>
                  <TableCell sx={{ textAlign: "center" }}>
                    {getStandings(currentSeasonStandingsData, driver, "playoff_16_wins") +
                      getStandings(currentSeasonStandingsData, driver, "playoff_12_wins") +
                      getStandings(currentSeasonStandingsData, driver, "playoff_8_wins") +
                      getStandings(currentSeasonStandingsData, driver, "champion")}
                  </TableCell>
                  <TableCell sx={{ textAlign: "center" }}>
                    {getStandings(currentSeasonStandingsData, driver, getCurrentRoundWinColumn())}
                  </TableCell>
                  <TableCell sx={{ textAlign: "center" }}>
                    {getStandings(currentSeasonStandingsData, driver, "playoff_points")}
                  </TableCell>
                </TableRow>
              );
            })}
