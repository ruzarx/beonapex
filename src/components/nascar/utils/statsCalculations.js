...
export const getStagePointsPercentage = (data, entity, racerType) => {
    ...
    const racerData = data.filter(race => race[colName] === entity);
    if (racerData.length === 0) return "-";
    const stagePoints = racerData.reduce((sum, race) => sum + race.stage_points, 0);
    const allPoints = racerData.reduce((sum, race) => sum + race.stage_points + race.finish_position_points, 0);
