...
const MainFantasyScreen = () => {
  ...
  useEffect(() => {
    ...
    const extractedGroups = [...new Set(raceData.map((entry) => entry[groupType]))]
      .filter(Boolean)
  ...
  useEffect(() => {
    if (selectedGroup) {
      ...
      const trackRaces = raceData
        .filter((entry) => entry.track_name === nextRaceTrack && entry.season_year >= 2022)
        .map((entry) => ({
      ...
      const similarRaces = raceData
        .filter((entry) => {
          ...
          return similarTracks.includes(entry.track_name) && entry.season_year >= 2022;
        })
        .map((entry) => ({
          season: entry.season_year,
          race_date: entry.race_date,
      ...
      const allPastSeasonRaces = raceData
        .filter(
          (entry) =>
            entry.season_year === currentSeason - 1 && entry.race_number <= currentRaceNumber
        ) // ✅ Properly closed
        .map((entry) => ({ race_date: entry.race_date }));
      setPastSeasonDates([...new Set(allPastSeasonRaces.map((race) => race.race_date))]);
