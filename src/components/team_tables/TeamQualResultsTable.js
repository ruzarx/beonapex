...
const TeamQualResultsTable = ({ teams, raceDates, similarRaceDates, allRaceDates, currentSeasonDates }) => {
  const [excludePlayoffs, setExcludePlayoffs] = useState(false);
  const [excludeDnf, setExcludeDnf] = useState(false);

  const sortedTeams = [...teams].sort((a, b) => {
    const avgA = getTeamFeatureValue(raceData, a, raceDates, excludePlayoffs, excludeDnf, "quali_pos");
    const avgB = getTeamFeatureValue(raceData, b, raceDates, excludePlayoffs, excludeDnf, "quali_pos");
          ...
          {sortedTeams.map((team, index) => (
            <TableRow key={index}>
              ...
              <TableCell sx={{ width: "80px", textAlign: "center" }}>
                {getTeamFeatureValue(raceData, team, raceDates, excludePlayoffs, excludeDnf, "quali_pos")}
              </TableCell>
              <TableCell sx={{ width: "80px", textAlign: "center" }}>
                {getTeamFeatureValue(raceData, team, similarRaceDates, excludePlayoffs, false, "quali_pos")}
              </TableCell>
              <TableCell sx={{ width: "80px", textAlign: "center" }}>
                {getTeamFeatureValue(raceData, team, allRaceDates, excludePlayoffs, false, "quali_pos")}
              </TableCell>
              <TableCell sx={{ width: "80px", textAlign: "center" }}>
                {getTeamFeatureValue(raceData, team, currentSeasonDates, excludePlayoffs, false, "quali_pos")}
              </TableCell>
                {raceDates.map((race_date, idx) => (
                  <TableCell key={idx} sx={{ width: "80px", textAlign: "center" }}>
                    {getTeamFeatureValue(raceData, team, race_date, excludePlayoffs, false, "quali_pos")}
                  </TableCell>
            ))}
            </TableRow>
          ))}
...
export default TeamQualResultsTable;
